...
export function AddDigitacaoModal({
    ...
    Promise.all([
      ...
      .then(([collabData, managerData]) => {
        ...
        if (collabData.collaborators) {
          ...
          for (const c of active) {
            ...
            if (groupLabel !== lastGroup) {
              result.push({ value: `__header_${groupLabel}`, label: groupLabel, icon: undefined });
        ...
        if (managers.length > 0) {
          ...
          result.push({ value: "__header_gerentes", label: "Gerentes", icon: undefined });
  ...
  // Focus input when entering add-clients step
  useEffect(() => {
    if (step === "add-clients") {
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div
            key="dig-panel"
            className="fixed bottom-0 left-0 right-0 z-[60] rounded-t-[2rem] border-t border-zinc-200 bg-white shadow-[0_-24px_80px_rgba(15,23,42,0.18)]"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
                ...
                {step === "select-collab" && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-amber-500 px-5 text-sm font-bold text-white shadow-md transition hover:bg-amber-600 disabled:opacity-50 active:scale-[0.98]"
                    >
                      Continuar →
                    </button>
                  </motion.div>
                ...
                {step === "add-clients" && (
                  ...
                  >
                    ...
                    <label className="grid gap-2">
                      ...
                      <span
                        className={cn(
                          "flex items-center gap-3 rounded-2xl border bg-white px-4 py-3 transition duration-200 focus-within:border-amber-400 focus-within:ring-4 focus-within:ring-amber-400/20",
